...
class GeminiClient:
    ...
    async def parse_exam_paper(self, text: str) -> Dict[str, Any]:
        ...
        2.  **題目整合**：若為考卷，依據「黃金準則」將關聯在一起的題組整合為單一題目物件。
        ...
        *   **核心概念：共享上下文 (Shared Context)**
            *   一份考卷的大題，通常包含兩部分：
                1.  **共享上下文**：在頂層題號（如「一、」）之後、第一個子問題（如「(一)」）之前的所有前導描述。
                2.  **子問題列表**：以 `(一)`、`(二)` 或 `(1)`、`(2)` 標示的多個具體問題。

        *   **分割與重組的執行演算法**：
            1.  **識別大題**：找到「一、」、「二、」等頂層題號。
            2.  **完整提取I**：將從一個頂層題號開始，直到下一個頂層題號出現之前（或文件結束）的所有內容，視為**一個完整的大題**。
            3.  **完整提取II**：如果出現承上題、接上題、延續上題、根據上題、基於前題等，無視頂層題號，將兩個大題目視為同一題。
        ...
        *   **準則 C：格式化規則**
            ...
            *   **語意重排版 (Readability Reformatting - 新增)**：
                ...
                3.  **優化排版**：如果這些區塊都擠在同一個段落，請使用換行和 Markdown 列表（例如，將 `(一)...(二)...` 轉換成一個清晰的點列或編號列表）來重新組織 `stem` 的內容，使其結構清晰、易於閱讀。
