...
function initializeContentTabs() {
    ...
    function formatOrganizationResult(result, type) {
        ...
        // 增強的結果驗證
        if (!result) {
            console.warn(`${type} 結果為空`);
            return `<div class="alert alert-warning">
                <i class="fas fa-exclamation-triangle me-2"></i>
                沒有收到AI整理結果，請重新生成
                <button class="btn btn-sm btn-outline-primary ms-2" onclick="regenerateOrganization('${type}', '${type}')">
        ...
        if (typeof result !== 'object') {
            console.warn(`${type} 結果不是有效對象，類型: ${typeof result}`);
            return `<div class="alert alert-warning">
                <i class="fas fa-exclamation-triangle me-2"></i>
                收到的資料格式不正確，請重新生成
                <button class="btn btn-sm btn-outline-primary ms-2" onclick="regenerateOrganization('${type}', '${type}')">
        ...
        // 檢查是否為錯誤回應
        if (result.error) {
            console.error(`${type} 返回錯誤:`, result.error);
            return `<div class="alert alert-danger">
                <i class="fas fa-exclamation-circle me-2"></i>
                AI整理時發生錯誤: ${result.error}
                <button class="btn btn-sm btn-outline-primary ms-2" onclick="regenerateOrganization('${type}', '${type}')">
                    <i class="fas fa-sync me-1"></i>重新生成
                </button>
            </div>`;
        ...
        try {
            ...
            // 根據不同的整理類型顯示特定內容
            html += formatSpecificTypeContent(result, type);

        } catch (error) {
            console.error(`處理 ${type} 結果時發生錯誤:`, error);
            return `<div class="alert alert-danger">
                <i class="fas fa-bug me-2"></i>
                處理AI整理結果時發生錯誤: ${error.message}
                <button class="btn btn-sm btn-outline-primary ms-2" onclick="regenerateOrganization('${type}', '${type}')">
                    <i class="fas fa-sync me-1"></i>重新生成
                </button>
                <details class="mt-2">
                    <summary class="small text-muted">技術詳情</summary>
        ...
        if (!html) {
            html = generateFallbackContent(result, type);
        }
    ...
    // 新增：專門處理特定類型內容的函數
    function formatSpecificTypeContent(result, type) {
        try {
            switch (type) {
                case 'mindmap':
                ...
                case 'format_enhance':
                    return formatEnhanceContent(result);
                default:
                    return '';
            }
        } catch (error) {
            console.error(`格式化 ${type} 內容時出錯:`, error);
            return `<div class="alert alert-warning">
                <i class="fas fa-exclamation-triangle me-2"></i>
                無法正確顯示 ${type} 內容，請重新生成
            </div>`;
        }
    }

    // 新增：生成後備內容
    function generateFallbackContent(result, type) {
        // 檢查是否有任何可用的內容欄位
        ...
        for (const field of possibleContentFields) {
            if (result[field]) {
                console.log(`使用 ${field} 欄位作為 ${type} 的備用內容`);
                if (typeof result[field] === 'string') {
                    return `<div class="ai-organized-content markdown-content mb-3">${renderMarkdown(result[field])}</div>`;
                    ...
                    return `<div class="mb-3">
                        ...
                        <details>
                            ...
                            <pre class="bg-light p-2 mt-2">${JSON.stringify(result[field], null, 2)}</pre>
                        </details>
                    </div>`;
                }
            }
    ...
    function formatHierarchicalContent(result) {
        let html = '<div class="hierarchical-container p-3 border rounded">';
    ...
    function formatQALearningContent(result) {
        let html = '<div class="qa-learning-container p-3 border rounded">';
    ...
    function formatFeynmanContent(result) {
        let html = '<div class="feynman-technique-container p-3 border rounded">';
    ...
    function formatMemoryPalaceContent(result) {
        let html = '<div class="memory-palace-container p-3 border rounded">';
    ...
    function formatComparisonContent(result) {
        let html = '<div class="comparison-container p-3 border rounded">';
    ...
    function formatMandalaNinegridContent(result) {
        let html = '<div class="mandala-ninegrid-container p-3 border rounded">';
    ...
    // 新增：格式化增強內容格式化
    function formatEnhanceContent(result) {
        if (result.formatted_content) {
            return `<div class="ai-organized-content markdown-content mb-3">${renderMarkdown(result.formatted_content)}</div>`;
        }
        return '';
    }
    ...
    function formatOrganizationResult(type, result) {
        ...
        try {
            // 委託給專門的格式化函數
            return formatSpecificTypeContent(result, type);
        } catch (error) {
            console.error(`Error formatting ${type} content:`, error);
            return generateFallbackContent(type);
        }
    }
