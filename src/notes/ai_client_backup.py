...
class NoteAIClient:
    ...
    async def _safe_generate_json(self, prompt: str, use_simple_model: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # Return a consistent error structure on any unexpected failure
            return {
                'error': f'AI 生成或解析失敗: {str(e)}',
                'raw_response': None,
                'parsing_note': '發生例外，請稍後重試或簡化輸入'
            }
    ...
    def format_and_enhance_content(self, content: str) -> Dict[str, Any]:
        """格式化與補強 - 整理格式、補充資料、修正錯字，直接返回格式化內容"""
        prompt = f"""
        ...
        請直接輸出格式化後的 Markdown 內容，不要包裝在 JSON 中，不要添加其他說明文字。
        """
        
        # 格式化與補強：使用輔助模型（簡單的格式處理）
        response = self._run_async(self.gemini_client.generate_text_async(prompt, use_simple_model=True))
        
        if response and response.get('text'):
            formatted_content = response['text'].strip()
            return {
                'success': True,
                'organized_content': formatted_content,
                'formatted_content': formatted_content  # 兼容性
            }
        else:
            return {
                'success': False,
                'error': '格式化失敗',
                'organized_content': content,  # 失敗時返回原內容
    ...
    # 此方法已被棄用，請使用 GhostAIClient.generate_smart_note_content
    def generate_smart_note_content(self, context: Dict[str, Any]) -> str:
        """
        [已棄用] 智能生成筆記內容功能已移至 GhostAIClient
        請使用 ghost_ai_client.GhostAIClient().generate_smart_note_content(context)
        """
        from .ghost_ai_client import GhostAIClient
        ghost_client = GhostAIClient()
        return ghost_client.generate_smart_note_content(context)
    ...
    def _generate_smart_note_without_question(self, context: Dict[str, Any]) -> str:
        ...
        user_prompt = context.get('user_prompt', '').strip()
        title = context.get('title', '')
        from_question = context.get('source_question') is not None
        
        # 檢查生成條件
        if from_question:
            # 從題庫過來的筆記：只要有prompt就可以生成
            if not user_prompt:
                return "請提供您希望AI幫助生成的筆記內容描述或學習重點。"
        else:
            # 新建筆記：只要有prompt就可以生成，可以沒有user_content
            if not user_prompt:
...
```"""
        ...
        prompt = f"""
你是專業的教育內容生成專家，請{task_description}。
...
請直接生成一份完整的 Markdown 格式學習筆記，必須包含以下結構：

# {title}

## 📚 核心知識與概念
[詳細解釋主題的核心知識點，提供清晰的定義和說明]

## 🔍 深入分析
[深入分析相關概念、原理或方法，提供多角度的理解]

{"## 📝 整合筆記內容" if user_content else "## 📝 詳細學習內容"}
{"[保留並擴充使用者的現有筆記內容，同時補充相關知識]" if user_content else "[根據使用者要求生成詳細的學習內容]"}
...
3. 適當使用 emoji 增強可讀性
4. 確保內容結構完整
5. {"保留並擴充使用者現有內容" if user_content else "從零開始生成完整內容"}
6. 直接輸出筆記內容，不要包含任何解釋或前言
"""
        
        try:
            response = self._run_async(self.gemini_client.generate_text_async(prompt, use_simple_model=False))
            if response and response.get('text'):
                return response['text'].strip()
            else:
                return self._generate_fallback_note(title, user_prompt, user_content)
        except Exception as e:
            print(f"Error in smart note generation: {e}")
            return self._generate_fallback_note(title, user_prompt, user_content)
...
"""
            ...
            # 根據來源與請求類型選擇模型：Ghost/簡單格式類任務 → 輔助模型
            ctx_source = (context.get('source') or '').lower()
            use_simple = bool(context.get('use_simple_model')) or ctx_source == 'ghost'
            req_l = (enhancement_request or '').lower()
            if ("格式" in enhancement_request or "標題" in enhancement_request or "縮寫" in enhancement_request or
                any(k in req_l for k in ["format", "typo", "spelling", "grammar", "punct"])):
                use_simple = True

            if use_simple:
                response = self._run_async(
                    self.gemini_client.generate_async_simple(prompt, is_json=False)
                )
                primary_used = False
            else:
                # 使用主模型生成內容（提高品質）
            ...
            if response:
                ...
                if len(generated_content) > len(enhancement_request) and not generated_content.startswith(enhancement_request):
                    return {
                        ...
                        'success': True,
                        'model_used': 'primary' if primary_used else 'simple'
                    }
                else:
